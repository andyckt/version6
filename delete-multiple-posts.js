// Script to permanently delete multiple posts from the database
...
const deletePostsFile = path.join(__dirname, 'src', 'data', 'deletepost.txt');
const postUrls = fs.readFileSync(deletePostsFile, 'utf8').split('\n').filter(Boolean);

// Extract post IDs from URLs
const postIds = postUrls.map(url => {
  const parts = url.split('/');
  return parts[parts.length - 1];
});

async function deleteMultiplePosts() {
  console.log(`Preparing to delete ${postIds.length} posts`);
