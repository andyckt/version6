...
export default function TestMerchantPosts() {
  const [merchantUsername, setMerchantUsername] = useState('qushuilanting');
  const [inputValue, setInputValue] = useState('qushuilanting');
  ...
  const { 
    ...
    totalPosts,
    refreshPosts
  } = useMerchantPosts(merchantUsername);

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    setMerchantUsername(inputValue);
  };

  return (
    <div className="container mx-auto p-4 max-w-2xl">
      <h1 className="text-2xl font-bold mb-6">Merchant Posts API Test</h1>
