...
interface MerchantPostsResponse {
  success: boolean;
  posts: MerchantPost[];
...
export function useMerchantPosts(username: string, initialLimit: number = 12) {
  const [limit] = useState(initialLimit);
  const [isRefreshing, setIsRefreshing] = useState(false);

  // Define the SWR key generator function
  const getKey = (pageIndex: number, previousPageData: MerchantPostsResponse | null) => {
    ...
    if (pageIndex === 0) return `/api/merchants/${username}/posts?limit=${limit}`;
    
    // Add cursor for next pages
    return `/api/merchants/${username}/posts?limit=${limit}&cursor=${previousPageData?.nextCursor}`;
  };
  ...
  const {
    ...
    isValidating,
    mutate
  } = useSWRInfinite<MerchantPostsResponse>(getKey, fetcher, {
