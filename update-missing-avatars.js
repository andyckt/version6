// Script to update users without profile images with a default male avatar
...
// Function to update users without profile images
async function updateMissingAvatars(imagePath) {
  ...
  try {
    ...
    // Find users without profile images
    // This includes users with null/undefined profileImage or empty string
    const usersWithoutImages = await usersCollection.find({
      $or: [
        { profileImage: null },
        { profileImage: "" },
    ...
    // Update all users without profile images with the new avatar
    const updateResult = await usersCollection.updateMany(
      { 
        _id: { 
          $in: usersWithoutImages.map(user => user._id) 
        } 
      },
      {
        $set: {
    ...
    console.log(`Successfully updated ${updateResult.modifiedCount} users with the default male avatar`);
    ...
    console.error('Error updating users without profile images:', error);
  } finally {
...
if (!imagePath) {
  console.error('Error: Please provide the path to the male avatar image');
  console.log('Usage: node update-missing-avatars.js <path-to-image>');
  process.exit(1);
}

// Run the function with the provided image path
updateMissingAvatars(imagePath).catch(console.error); 